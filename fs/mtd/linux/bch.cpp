 ...
 *   File name: bch.cpp
 *
 *  Created on: 2017年7月25日, 下午8:54:21
 *      Author: victor 
 *   Toolchain: 
 *    Language: C/C++
			      ...
			      unsigned int *roots)
{
	...
	for (i = 0; i < m; i++)
	{
		rows[i + 1] = bch->a_pow_tab[4 * i] ^ (a ? bch->a_pow_tab[mod_s(bch, k)] : 0)
				...
				unsigned int *roots)
{
	...
	if (poly->c[0] && poly->c[1])
	{
		...
		{
			roots[n++] = modulo(bch,
					2 * GF_N(bch) - l1 - bch->a_log_tab[r] + l2);
			roots[n++] = modulo(bch,
					2 * GF_N(bch) - l1 - bch->a_log_tab[r ^ 1] + l2);
		}
	}
	return n;
}
	...
	for (j = a->deg; j >= d; j--)
	{
		if (c[j])
		{
			...
			for (i = 0; i < d; i++, p++)
			{
				m = rep[i];
				if (m >= 0)
					c[p] ^= bch->a_pow_tab[mod_s(bch, m + la)];
			}
		}
	}
	a->deg = d-1;
...
static uint32_t *compute_generator_polynomial(struct bch_control *bch)
{
	...
	roots = (uint*)bch_alloc((bch->n+1)*sizeof(*roots), &err);
...
struct bch_control* UNUSED init_bch(int m, int t, unsigned int prim_poly)
{
	int err = 0;
	unsigned int i, words;
	uint32_t *genpoly;
	struct bch_control *bch = nullptr;
	...
	{
		0x25, 0x43, 0x83, 0x11d, 0x211, 0x409, 0x805, 0x1053, 0x201b, 0x402b,
		0x8003,
	};
	...
	bch = (struct bch_control *)kheap.allocate(sizeof(*bch));
	if (bch == nullptr)
		goto fail;

	zero_block(bch, sizeof(*bch));
	...
	bch->xi_tab    = (uint*)bch_alloc(m*sizeof(*bch->xi_tab), &err);
	bch->syn       = (uint*)bch_alloc(2*t*sizeof(*bch->syn), &err);
...
void UNUSED free_bch(struct bch_control *bch)
{
	unsigned int i;

	if (bch)
	{
		kheap.free(bch->a_pow_tab);
		kheap.free(bch->a_log_tab);
		kheap.free(bch->mod8_tab);
		kheap.free(bch->ecc_buf);
		kheap.free(bch->ecc_buf2);
		kheap.free(bch->xi_tab);
		kheap.free(bch->syn);
		kheap.free(bch->cache);
		kheap.free(bch->elp);

		for (i = 0; i < ARRAY_SIZE(bch->poly_2t); i++)
			kheap.free(bch->poly_2t[i]);

		kheap.free(bch);
	}
}

void UNUSED encode_bch(struct bch_control *bch, const uint8_t *data,
...
int UNUSED decode_bch(struct bch_control *bch, const uint8_t *data, unsigned int len,
	       ...
	       const unsigned int *syn, unsigned int *errloc)
{
	...
	if (!syn)
	{
		if (!calc_ecc)
		{
			if (!data || !recv_ecc)
				return -static_cast<sint>(ErrNo::EINVAL);
			encode_bch(bch, data, len, nullptr);
		}
		else
		{
	...
	return (err >= 0) ? err : -static_cast<sint>(ErrNo::EBADMSG);
}
